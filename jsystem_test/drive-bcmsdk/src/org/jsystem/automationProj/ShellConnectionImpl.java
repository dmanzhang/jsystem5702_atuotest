...
public abstract class ShellConnectionImpl extends SystemObjectImpl implements
    RawCliConnection {
	 ...
	 */
	RawIdleMonitor idleMonitor = null;
	...
	protected void navigate(RawCliCommand command, boolean toPosition) throws Exception {
	...
	public void navigateToPosition(RawCliCommand command) throws Exception {
		navigate(command, true);
	}

	public void returnFromPosition(RawCliCommand command) throws Exception {
	...
	public void connect() throws Exception {
		if (idleMonitor == null && maxIdleTime > 0) {
			idleMonitor = new RawIdleMonitor(this, maxIdleTime);
			idleMonitor.start();
		}
	...
	public void handleCliCommand(String title, RawCliCommand command) throws Exception {
		if (command.isClone()) {
			ShellConnectionImpl cloned = (ShellConnectionImpl) this.clone();
			try {
	 ...
	 */
	public static void handleCliCommand(ShellConnectionImpl cli, String title, RawCliCommand command) throws Exception {
	...
	public synchronized void command(RawCliCommand command) {
	...
	public Object clone() throws CloneNotSupportedException {
		try {
			ShellConnectionImpl newImpl = (ShellConnectionImpl) getClass().getClassLoader().loadClass(getClass().getName()).newInstance();
			newImpl.port = port;
			newImpl.user = user;
